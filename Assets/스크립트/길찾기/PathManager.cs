...
public class PathManager : MonoBehaviour
{
    ...
    [SerializeField]
    bool allowDiagonal, dontCrossCorner;
    ...
    public void pathFinding()
    {
        ...
        startPos =  Vector2Int.RoundToInt(this.gameObject.transform.position);
        targetPos =  Vector2Int.RoundToInt(inGameSceneCheckTargetAndGetDistance.target.transform.position);
            ...
            for (int j = 0; j < sizeY; j++)
            {
                ...
                foreach (Collider2D col in Physics2D.OverlapCircleAll(new Vector2(i , j ), 0.4f))
                {
                    ...
                    if (col.gameObject.layer == LayerMask.NameToLayer(this.gameObject.tag))
                    {
                        ...
                        if (otherCharState.nowState == CharState.NowState.isReadyForAttack)
                        {
                            isWall = true;
                        }
                    }

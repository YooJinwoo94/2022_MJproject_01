...
public class InGameSceneGameManager : MonoBehaviour
{
    ...
    public void changeIngameSpeed()
    {
        inGameSceneUiDataManager.inGameSpeedCount++;
        if (inGameSceneUiDataManager.inGameSpeedCount == 3) inGameSceneUiDataManager.inGameSpeedCount = 0;

        Time.timeScale = inGameSceneUiDataManager.inGameSpeed[inGameSceneUiDataManager.inGameSpeedCount];
    }




    public void ifEnemyCharDeadCheckLeftbattleSceneCount()
    {
        //이후에 레이드 수를 바꿀 예정 
        // 수정 예정
        if (inGameSceneUiDataManager.battleSceneCount == 2)
        {
            inGameSceneUiDataManager.nowGameSceneState = InGameSceneUiDataManager.NowGameSceneState.gameEnd;
            return;
        }
    ...
    public void ifPlayerCharDeadCheckLeftPlayerCharCount ()
    {
        //현재 남아있는 플레이어 케릭터가 없는 경우
    ...
    void setPlayerCharData()
    {
        for (int i = 0; i < PlayerDataJsonManager.instance.playerData.charGridData.nowGridData.GetLength(0); i++)
        {
            for (int k = 0; k < PlayerDataJsonManager.instance.playerData.charGridData.nowGridData.GetLength(1); k++)
            {
                ...
                int num = 0;
                num = k * PlayerDataJsonManager.instance.playerData.charGridData.nowGridData.GetLength(0) + i;

                turnOnPlayerChar(num, PlayerDataJsonManager.instance.playerData.charGridData.nowGridData[i, k]);
            }
        }
    }
    //적 캐릭터의 데이터를 가져온다.
    ...
    public void ifDragAndDrop(int dragStartNum, int dropStartNum, GameObject dragObj)
    {
        ...
        switch (inGameScenePlayerCharGridData.nowGridData[gridXYplayerWillPut[0], gridXYplayerWillPut[1]])
        {
            case null:
                ...
                inGameScenePlayerCharGridData.nowGridData[gridXYplayerWillPut[0], gridXYplayerWillPut[1]] = dragObjCharState.name;
            ...
            default:
                ...
                // 그리드 data 교환
                inGameScenePlayerCharGridData.nowGridData[gridXYplayerWillPut[0], gridXYplayerWillPut[1]] = dragObjCharState.name;
                inGameScenePlayerCharGridData.nowGridData[gridXYplayerWasPut[0], gridXYplayerWasPut[1]] = dropObjCharState.name;

...
{/*
    ...
    {    
        var request = new UpdateUserDataRequest() { Data = new Dictionary<string, string>() { { "현재 전투에 사용할 그리드", JsonConvert.SerializeObject(PlayerDataJsonManager.instance.playerData.charGridData.nowGridData) } } 
        };
    ...
    public void putAllGridData()
    {
        ...
        {
            Data = new Dictionary<string, string>() { { "배치한 모든 그리드의 정보값", JsonConvert.SerializeObject(PlayerDataJsonManager.instance.playerData.charGridData.allGridData) } }
        };
        PlayFabClientAPI.UpdateUserData(request, (result) => print("데이터 저장 성공"), (error) => print("데이터 저장 실패"));
    }
    ...
    */
    ...
    void playerLoginData(GetUserDataResult result)
    {
        ...
        {
            PlayerDataJsonManager.instance.playerData.charGridData.allGridData = JsonConvert.DeserializeObject<string[,,]>(result.Data["배치한 모든 그리드의 정보값"].Value);
            PlayerDataJsonManager.instance.playerData.charGridData.nowGridData = JsonConvert.DeserializeObject<string[,]>(result.Data["현재 전투에 사용할 그리드"].Value);
